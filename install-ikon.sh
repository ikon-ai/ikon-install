...
install_dotnet_via_script() {
    ...
    if ! grep -q 'DOTNET_ROOT="\$HOME/.dotnet"' "$shell_rc" 2>/dev/null; then
...
if ! dotnet tool update ikon --global --no-http-cache --configfile "$NUGET_CONFIG"; then
...
echo "Repairing Ikon tool packages..."
if ! ikon --repair; then
    echo -e "${YELLOW}Warning: Could not repair the Ikon tool packages; 'ikon login' will install them${NC}"
fi
